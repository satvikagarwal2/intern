...
def returnMailTypeAndTone(buyerMail, productTitle):
    ...
    prompt1.append(
        'reply should be in python list format [Mail Type, Mail Tone], so that i will directly typecast this output string object to python list object')
    ...
    mailTT_response = openai.Completion.create(engine=model,
                                               prompt=prompt_mailTT,
                                               max_tokens=2000,
                                               n=1,
                                               stop=None,
                                               temperature=temperature)

    response = mailTT_response.choices[0].text
    mailType = ''
    mailTone = ''
